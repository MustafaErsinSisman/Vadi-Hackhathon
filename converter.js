...
function professionalConverter(fileName) {
    return new Promise((resolve, reject) => {
        ...
        ffmpeg.ffprobe(inputPath, (err, metadata) => {
            if (err) {
                const errorObj = { status: "Failed", error: "Analysis Error" };
                fs.writeFileSync(path.join(outputBaseDir, 'metadata.json'), JSON.stringify(errorObj));
            ...
            command
                ...
                .on('error', (ffmpegErr) => {
                    const errorObj = { status: "Failed", error: ffmpegErr.message };
                    fs.writeFileSync(path.join(outputBaseDir, 'metadata.json'), JSON.stringify(errorObj));
                    console.error('[Converter] FFmpeg error', ffmpegErr);
                ...
                .on('end', () => {
                    ...
                    const stats = {
                        fileName,
                        outputDir: path.parse(fileName).name,
                        ...
                        resolutions: selectedQualities.map(q => ({ name: q.name, width: q.width, height: q.height })),
